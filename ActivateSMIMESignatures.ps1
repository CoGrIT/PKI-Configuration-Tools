<# 
ActivateSMIMESignatures.ps1 Version: 20240326
...
This enables the Sign and encrypt buttons of the Options dialogue when composing a message.
    ...
    .PARAMTER SubjectMatch
    Part of the SubjectName, which should be matched.d
	...
	.EXAMPLE
    .\ActivateSMIMESignatures.ps1 -CAName "CN=COMODO RSA Client Authentication and Secure Email CA, O=COMODO CA Limited, L=Salford, S=Greater Manchester, C=GB" -SubjectMatch $([Environment]::UserName) -AlwaysSignMails
...
20240326: Be sure to exit & add Subject Matching

#>
param
( 
[Parameter(Position=0,Mandatory=$true,ValueFromPipeline=$false,HelpMessage='exact name of Issuing CA')][string]$CAName,
[Parameter(Position=1,Mandatory=$true,ValueFromPipeline=$false,HelpMessage='check for the following subject part')][string]$SubjectMatch,
[Parameter(Position=2,Mandatory=$false,ValueFromPipeline=$false,HelpMessage='instruct Outlook to sign emails as default setting')][switch]$AlwaysSignMails,
[Parameter(Position=3,Mandatory=$false,ValueFromPipeline=$false,HelpMessage='always write new configuration data, even if some configuration data exists already (for example using an expired certificate)')][switch]$Force,
[Parameter(Position=4,Mandatory=$false,ValueFromPipeline=$false,HelpMessage='adds the certificate as encryption certificate, too, instead of only signatures')][switch]$EnableEncryption,
[Parameter(Position=5,Mandatory=$false,ValueFromPipeline=$false,HelpMessage='instruct Outlook to encrypt emails as default setting')][switch]$AlwaysEncryptMails
...
Class OutlookSignatureSettings {
    ...
    OutlookSignatureSettings([string] $OfficeBitness, [byte[]] $binExistingSettings) {
        ...
        if (1 -ne ([System.BitConverter]::ToUInt32($binExistingSettings,0))) {
            throw "Existing settings do not start with 01 00 00 00 as expected. Maybe multiple profiles?"
        }
...
# Main
Write-Information "ActivateSMIME Version 20240326"

## Search for an appropriate certificate
$sOidSecureEmail = "1.3.6.1.5.5.7.3.4"
$CandidateCerts = @(Get-ChildItem cert:\CurrentUser\My | Where-Object { $_.Subject -match $SubjectMatch -and $_.Issuer -eq $CAName -and $_.HasPrivateKey -and ( ( ($_.EnhancedKeyUsageList | Where-Object { $_.ObjectId -eq $sOidSecureEmail }) -ne $null) -OR ($_.Extensions| Where-Object {$_.EnhancedKeyUsages | Where-Object {$_.Value -eq $sOidSecureEmail} } ) ) })
